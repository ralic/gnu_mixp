...
handler should have been specified on this parser."
  (let ((parser (args->parser args "mixp:xml->list"))
    ...
    (expat:set-element-handler
     parser (lambda (data name attributes)
              (prepend! data `((start-element ,name ,attributes))))
     (lambda (data name)
       (prepend! data `((end-element ,name)))))
    (expat:set-character-data-handler
     parser (lambda (data value)
              (prepend! data `((character-data ,value)))))
    (expat:set-notation-decl-handler
     parser (lambda (data notation-name base system-id public-id)
              (prepend! data `((notation-decl ,notation-name ,base
                                              ,system-id ,public-id)))))
    (expat:set-unparsed-entity-decl-handler
     parser (lambda (data entity-name base system-id public-id notation-name)
              (prepend! data `((entity-decl ,entity-name ,base
                                            ,system-id ,public-id
                                            ,notation-name)))))
    ;; TBD: The following definitions introduce memory management errors
